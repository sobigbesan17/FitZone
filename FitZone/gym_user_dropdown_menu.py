...
class GymPagesDropdownMenu(tk.Frame):
    def __init__(
        ...
        self.gym_class_booking_callback = gym_class_booking_callback
        self.book_class_callback = book_class_callback
        self.gym_class_clashes_callback = gym_class_clashes_callback
    ...
    def _create_nav_tabs(self):
        tabs = [
            ...
            ("Reservations", "class_reservations"),
    ...
    def _handle_click(self, key):
        ...
        if not cb:
            if key == "dashboard":
                ...
                cb = self.calculate_bmi_callback
            elif key == "bmi_vis":
                cb = self.bmi_visualisation_callback
            elif key == "meal_planner":
                cb = self.gym_meal_planner_callback
            elif key == "workout_planner":
                cb = self.gym_workout_planner_callback
            elif key == "class_schedule":
                cb = self.view_class_schedule_callback
            elif key == "class_reservations":
                cb = self.view_class_reservations_callback
            elif key == "book_class":
                cb = self.gym_class_booking_callback or self.book_class_callback
            elif key == "class_clashes":
                cb = self.gym_class_clashes_callback
        if cb:
            cb()
